  ...
  protected function _buildManifestItem($record)
  {
    ...
    foreach ($elementTexts as $elementSetName => $elements) {
      foreach ($elements as $elementName => $values) {
        if ($elementName == 'XML' || strpos($elementName, 'Image') !== false) {
          $values = str_replace('http://localhost/uploads/', '', $values);
        }
        // if element has multiple values, combine them into a string
        $newValue = '';
        if (count($values > 0)) {
        ...
        $elementName = str_replace('Description', __('Description'), $elementName);
        $elementName = str_replace('Creator', __('Creator'), $elementName);
        $elementName = str_replace('Source', __('Source'), $elementName);
        $elementName = str_replace('Publisher', __('Publisher'), $elementName);
        $elementName = str_replace('Date', __('Date'), $elementName);
        $elementName = str_replace('Contributor', __('Contributor'), $elementName);
        $elementName = str_replace('Rights', __('Rights'), $elementName);
        $elementName = str_replace('Relation', __('Relation'), $elementName);
        $elementName = str_replace('Format', __('Format'), $elementName);
        $elementName = str_replace('Language', __('Language'), $elementName);
        $elementName = str_replace('Type', __('Type'), $elementName);
        $elementName = str_replace('Identifier', __('Identifier'), $elementName);
        $elementName = str_replace('Image', __('Image'), $elementName);
        $elementName = str_replace('Location', 'Kirjoituspaikka', $elementName);
    ...
    // sort metadata array: move 'Location' after 'Date'
    ...
    if ($metadata[1]->label == 'Kuvaus') {
      moveElement($metadata, 11, 4);
    } else {
      moveElement($metadata, 10, 3);
    }
