...
if (!empty($formActionUri)):
    ...
    /* Make sure action attribute value is '/gottlund/items/browse' on all pages */
    $url = url(array('controller'=>'items','action'=>'browse'));
    if ($url != '/gottlund/items/browse') {
      $url = str_replace($url, '/gottlund/items/browse', $url);
    }
    ...
    <div id="search-narrow-by-fields" class="field">
        ...
        foreach ($search as $i => $rows): ?>
            <?php
              ...
              $table_options['Dublin Core'] = array_diff($table_options['Dublin Core'],
                [__('Subject'), __('Description'), __('Coverage'), __('Type'), __('Format'), __('Publisher'), __('Rights'),
                __('Contributor'), __('Source'), __('Relation')]);

              $table_options['Aineistotyypin metatiedot'] = array_intersect($table_options['Aineistotyypin metatiedot'], array(__('Location')));
